...
export const MatchesProvider = ({ children }: iMatchesProvider) => {
  ...
  async function createMultipleTournamentMatches(
    tournamentId: number,
  ...
  async function deleteAllMatchesFromTournament(tournamentId: number) {
    try {
      await api
        .get<iMatchData[]>(`matches?&tournamentId=${tournamentId}`)
  ...
  async function updateMatchTeams(
    ...
    try {
      api.patch(`matches/${matchId}`, data).then(() => {
  ...
  async function updateMatchScores(
    ...
    try {
      api.patch(`matches/${matchId}`, data).then(() => {
        ...
        if (order === 1) {
          if (semiFinalsI) {
            updateMatchTeams(semiFinalsI.id, {
              teamA: {
                id: data.winner.id,
        ...
        if (order === 3) {
          if (semiFinalsII) {
            updateMatchTeams(semiFinalsII.id, {
              teamA: {
                id: data.winner.id,
                name: data.winner.name,
              },
        ...
        if (order === 5) {
          if (grandFinal) {
            updateMatchTeams(grandFinal.id, {
              teamA: {
                id: data.winner.id,
                name: data.winner.name,
              },
